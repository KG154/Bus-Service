...
class SiteProvider {
  ...
  ///get site list
  Future<SiteListModel?> getSiteList(String? userId, String? token, bodyData) async {
    final hasInternet = await checkInternets();
    if (hasInternet == null || !hasInternet) {
      MyToasts().errorToast(toast: Validate.noInternet);
      return null;
    } else {
      final url = "${Utils.baseUrl}/get_site_list";
      // final url = "$baseUrl/get_site_list";
      try {
        log("${userId.toString()} & ${token.toString()} & ${url}", name: "get_site_list HeaderData");
        ...
        if (response.statusCode == 200) {
          ...
          log(response.body);
          SiteListModel data = SiteListModel.fromJson(responseBody);
          return data;
  ...
  ///add site
  Future<SuccessModel?> addSite(bodyData) async {
    final hasInternet = await checkInternets();
    if (hasInternet == null || !hasInternet) {
      MyToasts().errorToast(toast: Validate.noInternet);
      return null;
    } else {
      final url = "${Utils.baseUrl}/add_site";
      // final url = "$baseUrl/add_site";
      try {
        LoginUserData? userModel = await Storage.getUser();
        log("${userModel?.userId} & ${userModel?.token} & ${url}", name: "add_site HeaderData");
  ...
  ///edit site
  Future<SuccessModel?> editSite(bodyData) async {
    final hasInternet = await checkInternets();
    if (hasInternet == null || !hasInternet) {
      MyToasts().errorToast(toast: Validate.noInternet);
      return null;
    } else {
      final url = "${Utils.baseUrl}/edit_site";
      // final url = "$baseUrl/edit_site";
      try {
        LoginUserData? userModel = await Storage.getUser();
        log("${userModel?.userId} & ${userModel?.token} & ${url}", name: "edit_site HeaderData");
        log(bodyData.toString(), name: "edit_site BodyData");
  ...
  Future<SuccessModel?> deleteSite(bodyData) async {
    final hasInternet = await checkInternets();
    if (hasInternet == null || !hasInternet) {
      MyToasts().errorToast(toast: Validate.noInternet);
      return null;
    } else {
      final url = "${Utils.baseUrl}/delete_site";
      // final url = "$baseUrl/delete_site";
      try {
        LoginUserData? userModel = await Storage.getUser();
        log("${userModel?.userId} & ${userModel?.token} & ${url}", name: "delete_site HeaderData");
        log(bodyData.toString(), name: "delete_site BodyData");
